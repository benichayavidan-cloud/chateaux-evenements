...
export function BlogIndexView({ page, posts: allPosts }: BlogIndexViewProps) {
  ...
  const filteredPosts = useMemo(() => {
    ...
    // Navigation par défaut : tri par date décroissante + tranche de la page courante
    ...
    <main className="brakt-blog min-h-screen bg-gradient-to-b from-gray-50 to-white flex flex-col items-center w-full">
      ...
      <div className="w-full flex justify-center items-center" style={{ padding: '12px 20px' }}>
        <div className="w-full max-w-7xl mx-auto flex flex-col items-center gap-12 sm:gap-16 md:gap-20">
          ...
          {/* Featured Post - Style Brakt (page 1 uniquement) */}
          {isBrowsing && page === 1 && (
            <div
          ...
          {/* Pagination SSR (navigation par défaut uniquement) */}
          {isBrowsing && totalPages > 1 && (
            <nav aria-label="Pagination des articles" className="flex items-center justify-center gap-3 flex-wrap">
              {page > 1 && (
                <Link
                  href={page === 2 ? "/blog" : `/blog/page/${page - 1}`}
                  className="inline-flex items-center gap-2 rounded-full border-2 border-gray-200 bg-white text-gray-700 hover:border-[var(--bronze-antique)] hover:text-[var(--bronze-antique)] font-medium text-sm transition-all duration-300"
                  style={{ padding: "10px 20px" }}
                >
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((n) => (
                <Link
                  ...
                  className={`inline-flex items-center justify-center rounded-full font-medium text-sm transition-all duration-300 ${
                    n === page
                      ? "bg-[var(--bronze-antique)] text-white shadow-lg"
                      : "bg-white text-gray-700 border-2 border-gray-200 hover:border-[var(--bronze-antique)] hover:text-[var(--bronze-antique)]"
                  }`}
