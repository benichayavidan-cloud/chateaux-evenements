    ...
    <>
        ...
        <Container size="xl">
          ...
          <div style={{ display: 'flex', gap: 'clamp(1rem, 2vw, 2rem)', maxWidth: '1400px', margin: '0 auto', overflowX: 'auto', justifyContent: 'center', padding: '3rem 0', minHeight: '380px' }}>
            {chateau.atouts.map((atout: string, index: number) => (
              <motion.div
                key={index}
                initial={{ opacity: 0, scale: 0.5, rotateY: -180, rotateZ: -10 }}
                whileInView={{ opacity: 1, scale: 1, rotateY: 0, rotateZ: 0 }}
                transition={{ duration: 0.8, delay: index * 0.15, ease: [0.34, 1.56, 0.64, 1], type: "spring", stiffness: 100 }}
                whileHover={{ scale: 1.05, rotateY: 5, y: -10, transition: { duration: 0.3 } }}
                viewport={{ once: true }}
              ...
              >
                ...
                >
                  <motion.div
                    ...
                    style={{ position: 'absolute', inset: 0, background: `linear-gradient(90deg, transparent 0%, rgba(163, 126, 44, 0.15) 30%, rgba(212, 175, 55, 0.25) 50%, rgba(163, 126, 44, 0.15) 70%, transparent 100%)`, pointerEvents: 'none', zIndex: 1 }}
                  />
                  <div
        ...
        <Container size="xl">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 'clamp(1rem, 2vw, 1.5rem)', maxWidth: '1400px', marginLeft: 'auto', marginRight: 'auto' }}>
            {chateau.images.galerie.slice(0, 6).map((image, index) => (
              ...
              >
                <Image src={image} alt={`${chateau.nom} - Image ${index + 1}`} fill sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 33vw" className="object-cover" loading="lazy" quality={85} style={{ transition: 'transform 0.6s cubic-bezier(0.4, 0, 0.2, 1)' }} />
                <motion.div
                  initial={{ opacity: 0 }}
      ...
      <Section background="gray" style={{ padding: '20px 0', background: '#f6f9fc' }}>
        <Container size="lg">
                    ...
                    <AnimatePresence>
                      {isOpen && (
                        <motion.div
                          ...
                          animate={{ height: 'auto', opacity: 1, transition: { height: { duration: 0.4, ease: [0.4, 0, 0.2, 1] }, opacity: { duration: 0.3, delay: 0.1 } } }}
                          exit={{ height: 0, opacity: 0, transition: { height: { duration: 0.3, ease: [0.4, 0, 0.2, 1] }, opacity: { duration: 0.2 } } }}
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.4 }} viewport={{ once: true }} style={{ textAlign: 'center', marginTop: theme.spacing['4xl'] }}>
