...
/**
 * Script de renommage SEO professionnel - Château Mont Royal
 * Renomme toutes les 19 images avec des noms descriptifs optimisés
 */
...
const MAPPING_FILE = path.join(__dirname, 'rename-mont-royal-seo.json');

async function main() {
  console.log('╔═══════════════════════════════════════════════════════════╗');
  console.log('║  🏰 Renommage SEO - Château Mont Royal (19 Images)       ║');
  console.log('╚═══════════════════════════════════════════════════════════╝\n');
  ...
  try {
    ...
    // Traiter Château Mont Royal
    for (const [folderKey, renames] of Object.entries(mapping)) {
      const folderName = 'Chateau Mont Royal';
      ...
      for (const [oldName, newName] of Object.entries(renames)) {
        ...
        try {
          ...
          console.log(`   ✅ ${oldName}`);
          console.log(`      → ${newName.replace('chateau-mont-royal-60-oise-chantilly-', '')}`);
